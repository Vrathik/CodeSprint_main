  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {verificationStatus === 'success' && verificationResult && (
        <div className={`bg-${verificationResult.wasteTypeMatch && verificationResult.quantityMatch ? 'green' : 'yellow'}-50 border-l-4 border-${verificationResult.wasteTypeMatch && verificationResult.quantityMatch ? 'green' : 'yellow'}-400 p-4 mb-8`}>
          <div className="flex">
            <div className="flex-shrink-0">
              <CheckCircle className={`h-5 w-5 text-${verificationResult.wasteTypeMatch && verificationResult.quantityMatch ? 'green' : 'yellow'}-400`} />
            </div>
            <div className="ml-3">
              <h3 className={`text-sm font-medium text-${verificationResult.wasteTypeMatch && verificationResult.quantityMatch ? 'green' : 'yellow'}-800`}>
                Verification Complete
              </h3>
              <div className={`mt-2 text-sm text-${verificationResult.wasteTypeMatch && verificationResult.quantityMatch ? 'green' : 'yellow'}-700`}>
                <p>{getVerificationMessage()}</p>
                <p className="mt-1">Confidence: {(verificationResult.confidence * 100).toFixed(2)}%</p>
              </div>
            </div>
          </div>
        </div>
